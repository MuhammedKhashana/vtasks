...
import { Tabs, Card, Text, Separator, Flex, Dialog, Button, TextField } from '@radix-ui/themes'
import { addTaskInMain, deleteTaskInMain, isCompletedTaskInMain, updateTaskInMain } from '../RTK/Slices/mainHeadSlice'
import { Check2Square, CheckLg, ListCheck, PencilSquare, PlusLg, Trash3Fill } from 'react-bootstrap-icons'
import { Fade, Zoom } from "react-awesome-reveal";
import Accordion from 'react-bootstrap/Accordion';


export default function MainHeadTasks() {
    const dispatch = useDispatch()

    const mainHeadTasks = useSelector((state) => state.mainHead)

    const [completedTasksN, setCompletedTasksN] = useState(0);

    useEffect(() => {
        const numberOfCompletedTasks = mainHeadTasks.map((task) => {
    ...
    }, [mainHeadTasks]); // Make sure to specify dependencies for useEffect if needed
    ...
    return (
        <Tabs.Content value='mainHead'>
            ...
            <Zoom duration={'300'}>
                {
                    (mainHeadTasks.length !== completedTasksN) ?
                        <Card >
                            {
                                mainHeadTasks.map((task) => (
                                                ...
                                                mainHeadTasks.map((task) => (
                                                    (!task.data.isCompleted) ?
                                                        ...
                                                        <div key={task.id} className='mb-3'>
                                                            <div className='d-flex flex-row justify-content-between align-items-center'>
                                                                <div className='d-flex flex-row justify-content-between align-items-center'>
                                                                    <button className='btn' onClick={() => dispatch(isCompletedTaskInMain({
                                                                        taskID: task.id,
                                                                        isCompleted: false
                                                                    }))}>
                                                                ...
                                                                <div>
                                                                    <button className='btn' onClick={() => dispatch(deleteTaskInMain(task.id))}><Trash3Fill size='20' /></button>
                                                                </div>
                                                            </div>
                                                            <Separator my="3" size="4" />
                                                        </div>
